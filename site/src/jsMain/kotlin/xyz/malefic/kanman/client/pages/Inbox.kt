...
fun Inbox(ctx: PageContext) =
    ctx.Request(request = { getInvitations() }) { invitations ->
        Column(
            Modifier
                .fillMaxSize()
            ...
            Column(
                ...
                invitations.forEach { invitation ->
                    Row(
                        ...
                        MdiSend(Modifier.scale(5))
                        Column(Modifier.gap(4.px)) {
                            ctx.Request(request = { getBoard(invitation.boardId) }) { board ->
                                H3 { Text(board.title) }
                            }
                            ctx.Request(request = { getUser(invitation.senderId) }) { sender ->
                                P(Modifier.opacity(0.7).toAttrs()) { Text(sender.username) }
                            }
                        }
                        MdiCheckCircle(
                            Modifier.scale(5).cursor(Cursor.Pointer).onClick { ctx.router.navigateTo("/boards/${invitation.boardId}") },
                        )
                    }
                }
            }
